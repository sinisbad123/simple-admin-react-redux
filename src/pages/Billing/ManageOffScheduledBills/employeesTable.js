...
class EmployeesTable extends Component {
  ...
  componentWillMount() {
    if (this.props.fetchTerminatedEmployees) {
      this.props.fetchTerminatedEmployees();
    }
  }
  ...
  onPageChange = (page, sizePerPage) => {
    if (this.props.fetchTerminatedEmployees) {
      if (this.state.searchKeyword && this.state.searchKeyword.trim() !== '') {
        this.props.fetchTerminatedEmployeesByName(page, this.state.searchKeyword);
        return;
      }

      this.props.fetchTerminatedEmployees(page);
      return;
    }
  }
  ...
  onSearchButtonClick = () => {
    if (this.props.fetchTerminatedEmployeesByName) {
        const keyword = this.state.searchKeyword;
        if (keyword && keyword.trim().length !== 0) {
            this.props.fetchTerminatedEmployeesByName(1, keyword);
            return;
        }

        if (!keyword || keyword.trim() === '') {
            this.props.fetchTerminatedEmployees(1);
            return;
        }
    }
  }
